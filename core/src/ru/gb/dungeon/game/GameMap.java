...
public class GameMap {
    ...
    public static final int CELLS_SIZE = 60;
    ...
    public void render(SpriteBatch batch) {
        for (int i = 0; i < CELLS_X; i++) {
            for (int j = CELLS_Y - 1; j >= 0; j--) {
                batch.draw(grassTexture, i * CELLS_SIZE, j * CELLS_SIZE);
                if (data[i][j].type == CellType.TREE) {
                    batch.draw(treesTextures[data[i][j].index], i * CELLS_SIZE, j * CELLS_SIZE);
                }
            }
        }
    }
